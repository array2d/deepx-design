...
4. **No VM heartbeat under `/sys/`.** The `/sys/` section says the registry holds the "VM heartbeat" and the op-operator list. The Go source implements the operator registry (`/sys/op/<backend>/<n>`, `/sys/op/<backend>/<n>/cmd`, `/sys/op/<backend>/func/<opname>` via `keytree/sys.go` and `rwir/dispatch/router.go`, plus `/rwir/<opcode>` via `rwir/builtin/ops.go`), but no heartbeat key. The only `/sys/vm/` key is `/sys/vm/<vmID>/err` for system-level error notification (`kvcpu/cpu.go`). The "VM 心跳" claim is not reflected in the implementation.
...
- `/sys/` op-operator registry: `keytree/sys.go` (`SysOpRoot`, `SysOpCmd`, `SysOpFunc`, `RwirRoot`).

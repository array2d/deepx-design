...
| 归属 | 存放位置 | 举例 |
|------|---------|------|
| **函数签名** | `/lib/<pkg>.<name>` — `string:rwfunc func(args) -> (rets)` | `rwfunc add(A:int64, B:int64) -> (C:int64)` |
...
- 签名中的 `name:type` 的 `type` 用**签名类型表达式**（联合/并集/高维 shape/动态维，支持多态），见 [[runtime篇-07-签名类型表达式]]

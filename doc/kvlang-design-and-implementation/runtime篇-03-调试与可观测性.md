# Debugging and Observability（调试与可观测性）
 ...
 更新于 2026-07-30：.funclib → .lib（对齐 scope 帧重设计）。
...
**`.lib` 软链 = 零拷贝共享指令树**。四个帧的 `.lib` 均 Link 到 `/lib/<name>`。关键：f3 帧出现在两个位置——`/vthread/{vid}/[0,0]/[0,0]/[1,0]`（f2 的子帧）和 f2 `.lib` 展开树中——这是因为 `kvspace tree` 跟随软链展开了 `/lib/f3` 的内容。实际上 KV 存储中只存一份指令树，所有帧通过 Link 共享。
...
│   ├── [3]  rwir:v0  rwir:v1  rwir:+  int64:0
│   └── [4]  int64:0  int64:0  rwir:return
├── .init.src  string:rwfunc init() -> () { … }   ← 源码副本

...
用户 Python 代码 (torch-like API)
  ...
  nn.functional.leaffunc_*  →  create_A_B_tf_C 工厂函数
        ...
        │                           │
        ▼                           ▼
  KvirIR("add", [a, b], [out])  序列化
        │
        ▼
  scheduler.send(ir)  →  UDPConn.sendto("localhost:9090")
        │
        ▼
  [外部调度器]  →  kvspace /src/func/  →  VM 拾取执行
```

**关键特征：即刻发射 (Eager Emission)**

每一条 Python 层的张量操作，立即生成一条 `KvirIR` 字符串并通过 UDP 发送。
**没有图构建阶段、没有延迟发射、没有函数边界。**

### 1.2 代码生成核心：rtf 模块

`front/py/kvlang/nn/functional/rtf.py` — 所有 IR 发射的汇聚点：

```python
# rtf.py — 5 种发射模板，覆盖全部当前算子

def A_B_op_C(op, a, b, out, author):          # add(A, B) -> C
    ir = KvirIR(op, [tensor(a), tensor(b)], [tensor(out)])
    send(ir)

def A_op_C(op, a, out, author):                # relu(A) -> C
    ir = KvirIR(op, [tensor(a)], [tensor(out)])
    send(ir)

def A_scalar_op_C(op, a, b, out, author):      # addscalar(A, 2.0) -> C
    ir = KvirIR(op, [tensor(a), varnum(b)], [tensor(out)])
    send(ir)

def A_B_c_op_D(op, a, b, c, out, author):      # equal(A, B, ε) -> D
    ir = KvirIR(op, [tensor(a), tensor(b), varnum(c)], [tensor(out)])
    send(ir)

def A_b1_b2_op_C(op, a, b1, b2, out, author):  # reduce(A, dims, keepdim) -> C
    ir = KvirIR(op, [tensor(a), vector(b1), varbool(b2)], [tensor(out)])
    send(ir)
```
...
`front/py/kvlang/tensor/elementwise.py` — Tensor 方法定义：

```python
@tensor_method
def add(self, other, out='') -> Tensor:
    from kvlang.nn.functional import add as add_func
    return add_func(self, other, out)  # → leaffunc → rtf → send
```

`front/py/kvlang/tensor/tensor.py` — Python 操作符重载：
...
`front/py/kvlang/nn/modules/module.py` — Module 基类：

```python
class Module:
    def register_parameter(name, param):
        # 注册参数时触发 rnewtensor(param)
        from kvlang.nn.functional.leaffunc_life import rnewtensor
        rnewtensor(param)
...
`front/py/kvlang/nn/modules/linear.py` — Linear 的 forward：
...
`front/py/kvlang/nn/kvir.py` — `Param` 类型标记：

| Param 类别 | Python 类型 | KvirIR 字符串形式 | 用途 |
|-----------|-----------|-------------------|------|
...
│                    即刻发射 (Eager)                    │
│                                                      │
│  Python op → rtf → KvirIR → UDP → 调度器 → kvspace     │
│                                                      │
│  ❌ 无计算图缓存         ❌ 无延迟发射                │
│  ❌ 无函数边界感知       ❌ 无控制流抽象              │
...
| | 结构化控制流 | 动态控制流 |
|------|-----------|---------|
| 决定时机 | 图构建期（Python 运行时） | kvir 执行期（VM 运行时） |
| 依赖数据 | Python 值（`None`/`int`/`bool`） | Tensor 值（在 kvspace/gpu 上） |
| 当前支持 | ✅ 天然支持（如 `if self.bias is not None`） | ❌ 不支持 |
| 需要 IR | 不需要（Python 本身处理） | 需要 kvir `if`/`for`/`while` |
...
| 输出格式 | 单条 KvirIR → UDP | C1 关键字 IR → kvspace `/src/func/<name>` |
...
def compile(func):
    """
    将 Python 函数编译为 kvir 函数定义，写入 kvspace /src/func/<name>。
    """
...
def example(x: Tensor, n: int) -> Tensor:
    ...
    for i in range(10):               # Python range → 编译期展开为顺序指令序列
        y = y + x                     #    （不需要 kvir for 控制流）

    # === for (动态边界) ===
    for _ in kvir.loop(cond=lambda: y.sum() < threshold):
        y = y * 0.9 + x * 0.1
...
│          if x > 0:           → IfContext 捕获                │
│              ...             → SymbolicTensor 拦截            │
│          return y            → finalize → kvlang → kvspace     │
│                              ✅ 编译模式（新增）              │
└──────────────────────────────────────────────────────────────┘
```
...
## 5. kvspace 存储格式（C1 关键字 IR）

编译器输出的 C1 关键字 IR 在 kvspace 中以嵌套 key 表达：
...
**关键属性**：IR 的嵌套结构与 kvspace key 的嵌套结构完全同构。
...
Go 前端 (`front/go/kvlang/`) 采用 **图模式 (Graph Mode)**：
...
| 控制流 | Python if/for（构建期） | kvir if/for/while（执行期） | Go if/for（构建期） |
| 输出格式 | 单条 IR → UDP | 完整 kvlang → kvspace | DOT 图 → 文件 |
...
3. 实现 `@kvir.compile` 装饰器（无分支，纯线性）
4. kvspace 写入 (`/src/func/<name>`)
...
TorchScript 需要维护一个完整的 Python 子集编译器（类型推断、控制流分析、字节码拦截），复杂度极高。kvir 只需要支持张量运算 + 基本控制流，拦截 `Tensor` 操作 + 控制流关键字就足够了。

### 为什么保留即刻模式

即刻模式（`rtf → KvirIR → UDP`）是调试、探索式开发、单步测试的基础。编译模式是性能优化和复杂控制流的手段。两种模式互补而非替代。

### 为什么函数是编译单元

以**函数**为编译单元而不是整个程序，因为：
1. kvir 的函数就是 VM 的调用/调度单元

...
- **rwir** 是最小执行单元，不可再分。解释器（builtin `Call()`）或后端引擎（`dispatch.Compute`）原子执行。rwir 是**空函数体**（仅签名，无指令体），声明存于 `/rwir/{runtime}/<opcode>`，`{runtime}` 反射自可执行文件名（如 `kvlang`），运行时签名可供 agent 自省。

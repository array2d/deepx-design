...
1. **rwir**（kvcpu 直接执行）：`writes <- opcode(reads...)` 或 `opcode(reads...) -> writes`。读参写参由箭头方向决定，无隐式栈、无匿名寄存器。`writes = expr` 是 `<-` 的等价书写（写槽在左）；与其它语言的 `=` 不同，读/写角色仍由指令形态严格约束，`=` 不是表达式、不可嵌套在条件中。
...
3. **控制流原语**（rwir子集）：`call`/`return`/`br`/`goto`——改变 PC。控制流详见 [parser篇-03 — 控制流](parser篇-03-控制流.md)。
